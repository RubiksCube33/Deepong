...
public class CourtManager : MonoBehaviourPunCallbacks
{
    ...
    // 게임 종료 시 플레이어들을 초기 위치로 이동 (ScoreManager에서 호출)
    public void ResetPlayersToInitialPositions()
    {
        if (player1 == null || player2 == null || player1Position == null || player2Position == null)
        {
            Debug.LogWarning("플레이어 오브젝트 또는 위치가 설정되지 않았습니다!");
            return;
        }
        
        // 네트워크 환경에서는 마스터 클라이언트만 위치 변경
        if (PhotonNetwork.IsConnected && !PhotonNetwork.IsMasterClient)
        {
            return;
        }
        ...
        // 플레이어 움직임 정지 (Rigidbody가 있는 경우)
        StopPlayerMovement(player1);
        StopPlayerMovement(player2);
        
        Debug.Log("플레이어들이 초기 위치로 이동되었습니다.");
        
        // 네트워크 동기화
        if (PhotonNetwork.IsConnected && photonView != null)
        {
            photonView.RPC("SyncPlayerReset", RpcTarget.Others);
        }
    }
    ...
    [PunRPC]
    void SyncPlayerReset()
    {
        // 다른 클라이언트에서 플레이어 위치 및 움직임 정지 동기화
        if (player1 != null && player2 != null && player1Position != null && player2Position != null)
        {
            player1.transform.position = player1Position.position;
            player2.transform.position = player2Position.position;
